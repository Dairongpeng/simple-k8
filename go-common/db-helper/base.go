...
func (h *DbTable) UpdateWhere(whereCause WhereCause, updateFields UpdateFields, insertIfNotExists bool) (err error) {
	if whereCause == nil {
		return fmt.Errorf("Missing where causes")
	}

	dest := UpdateFields{}
	if err = h.SelectOneWhere(updateFields.GetKeys(), whereCause).MapScan(dest); err == sql.ErrNoRows {
